...
function EventGuests({route, navigation}) {
    ...
    const guestsFiltered = guests.filter(oneGuest => {
        return oneGuest.firstName.toLowerCase().includes(keyword.toLowerCase())
            || oneGuest.lastName.toLowerCase().includes(keyword.toLowerCase());
    });

    const guestsSorted = guestsFiltered.sort((a, b) => {
        const aSlug = slugify(`${a.firstName.toLowerCase()}${a.lastName.toLowerCase()}`);
        const bSlug = slugify(`${b.firstName.toLowerCase()}${b.lastName.toLowerCase()}`);
