...
function EventGuests({ route, navigation }) {
  ...
  const guestsFiltered = guests.filter((oneGuest) => {
    result =
      oneGuest.lastName.toLowerCase().includes(keyword.toLowerCase()) ||
      oneGuest.firstName.toLowerCase().includes(keyword.toLowerCase());
    return result;
  });
  ...
  const manualScan = (idOfGuest) => {
    const invitationPublicId = event.invitation?.publicId;
        ...
        <>
          <View style={styles.statistics}>
            <View style={styles.statisticsItemContainer}>
              <Text
                style={styles.guests}
              >{`${event.guests.length} invités`}</Text>
            </View>
            <Progress.Bar
              height={18}
              ...
              progress={
                (
                  Math.round(
                    (isNaN(scans.length / guests.length)
                      ? 0
                      : scans.length / guests.length) * 100
                  ) / 100
                ).toFixed(2) * 2
              }
              width={null}
